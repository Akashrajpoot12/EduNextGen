...
export function ParentFeesPage() {
  ...
  const { tenantId: schoolId, isLoading: tenantLoading } = useTenant();
    ...
    try {
      ...
      const options = {
        ...
        handler: async function (paymentRes: any) {
          ...
          try {
            // Call Edge Function to verify payment signature and update database status
            ...
            if (verifyData.success) {
              toast.success(`Payment of ₹${amount.toLocaleString('en-IN')} Successful!`, { id: verifyToastId });
              // Optimistic UI Update
              setInvoices((prev) => 
                prev.map(inv => inv.id === invoiceId ? { ...inv, status: 'paid', payment_date: new Date().toISOString() } : inv)
              );
            } else {
              toast.error("Payment verification failed! " + (verifyData.error || ""), { id: verifyToastId });
            }
          } catch (err) {
            console.error(err);
      ...
      rzp1.on("payment.failed", function (paymentFailRes: any) {
        toast.error(`Payment Failed: ${paymentFailRes.error.description}`);
      });
      rzp1.open();
    } catch (err) {
      console.error(err);
